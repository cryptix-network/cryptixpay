document.addEventListener('DOMContentLoaded', async function () {
  ...
  const paymentId = (await (await fetch(`http://cryptixpay.kaffinp.xyz/api/createPayment?amount=${urlParams.get('amount')}&recipient=${urlParams.get('recipient')}`)).json()).result.paymentId
  const payment = (await (await fetch(`http://cryptixpay.kaffinp.xyz/api/getPayment?paymentId=${paymentId}`)).json()).result
  ...
  const checkLoop = setInterval(async () => {
    ...
    const paymentCheck = (await (await fetch(`http://cryptixpay.kaffinp.xyz/api/getPayment?paymentId=${paymentId}`)).json()).result
